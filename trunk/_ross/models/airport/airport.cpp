...
event_handler(airport_state * s, tw_bf * bf, airport_message * msg, tw_lp * lp)
{
  ...
  switch(msg->type)
    ...
    s->planes_in_the_sky++;//constructive operation
...
void
fw_event_handler_ss(airport_state * s, tw_bf * bf, airport_message * msg, tw_lp * lp)
{    
    int rand_result;
    tw_lpid dst_lp;
    tw_stime ts;
    ...
    switch(msg->type)
    {
        ...
        {
            // Schedule a landing in the future	
            __store__(s->planes_in_the_sky, lp);
            s->planes_in_the_sky++;
            
            __store__(s->furthest_flight_landing, lp);
            msg->saved_furthest_flight_landing = s->furthest_flight_landing;
        ...
        case DEPARTURE:
        {
            __store__(s->planes_on_the_ground, lp);
            s->planes_on_the_ground--;
        ...
        case LAND:
        {
            ...
            __store__(s->landings, lp);
            s->landings++;
            
            __store__(s->waiting_time, lp);
            s->waiting_time += msg->waiting_time;
...
void
rc_event_handler_ss(airport_state * s, tw_bf * bf, airport_message * msg, tw_lp * lp)
{    
    switch(msg->type)
    {
        case ARRIVAL:
            ...
            __restore__(s->furthest_flight_landing,lp);
            __restore__(s->planes_in_the_sky,lp);
...
//            s->furthest_flight_landing = msg->saved_furthest_flight_landing;
        ...
        case DEPARTURE:
            bs_rand_rvs(s->rn, lp);
            bs_rand_rvs(s->rn, lp);
            __restore__(s->planes_on_the_ground,lp);
        ...
        case LAND:
            ...
            __restore__(s->waiting_time,lp);
            __restore__(s->landings,lp);
            __restore__(s->planes_in_the_sky,lp);
            __restore__(s->planes_on_the_ground,lp);
...
{    
    ...
    switch(msg->type)
    {
        ...
        case ARRIVAL:
        {
            ...
            if (ts > 1000) 
            {
                cout << tw_rand_exponential(lp->rng, MEAN_LAND) << ' ';
                cout << ">>>>>>>>>>>> " << ts << '\n';
            }
...
//            rand_result = tw_rand_integer(lp->rng, 0, 3);
        ...
        case LAND:
        {
            ...
            double d = bs_rand_exponential2(s->rn, MEAN_DEPARTURE, lp);
            if (d > 1000) 
            {
                cout << tw_rand_exponential(lp->rng, MEAN_DEPARTURE) << ' ';
                cout << ">>>>>>>>>>>> " << d << '\n';
            }
            e = tw_event_new(lp->gid, d, lp);
