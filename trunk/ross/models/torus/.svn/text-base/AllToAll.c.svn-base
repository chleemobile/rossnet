...
packet_generate(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  ...
  {
  //      if(lp->gid==0)
  //	printf("%d Generating packet %d\n",N_nodes,s->generate_counter);
      // Send the packet out
      ts = tw_rand_exponential(lp->rng, 10);
      e = tw_event_new(lp->gid, ts, lp);
      m = tw_event_data(e);
